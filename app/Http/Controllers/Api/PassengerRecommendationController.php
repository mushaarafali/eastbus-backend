...
class PassengerRecommendationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($buses as $bus) {
            ...
            if ($availability['available_seats'] > 0) {
                $reasons[] = $availability['available_seats'] . ' seats available';
            }
            ...
            $tripData = [
                ...
                'fixed_service_id' => $upcomingTrip->fixed_service_id !== null
                    ? (int) $upcomingTrip->fixed_service_id
                    : null,
    ...
    private function findUpcomingTripForBus(int $busId)
    {
        $trips = DB::table('trips as t')
            ...
            ->select(
                ...
                'fs.is_active as service_is_active',
                'fs.is_published as service_is_published',
                'o.company_name as operator_name'
            )
    ...
    private function bookingCloseTime($trip): Carbon
    {
        if (!empty($trip->booking_closed_at)) {
            return Carbon::parse($trip->booking_closed_at);
        }
